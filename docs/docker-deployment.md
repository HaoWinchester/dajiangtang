...
> 重要说明：当前 Java 后端代码仍使用内存 Repository，暂未接入 JDBC/MySQL。因此 MySQL 容器用于保存本次整理出的表结构和初始化数据，方便后续改造成真实数据库持久化；当前线上页面和接口运行不依赖 MySQL 数据。
...
1. 当前后端不读取 MySQL，真正生产持久化需要新增 JDBC/JPA/MyBatis 数据访问层，并把内存 Repository 替换成数据库 Repository。
2. 不建议长期对公网开放 3306；优先使用安全组白名单或 SSH 隧道。
3. `.env` 不要提交到公开仓库，服务器上单独维护。
4. 第一次初始化 MySQL 后，如果修改了 `database/dajiangtang_dump.sql`，需要删除旧数据卷或手动导入，容器不会自动重复执行初始化脚本。
